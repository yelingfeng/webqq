...
appManagerPanel = function(me){
	...
	return me = {
		...
		bindEvent:function(){
			...
			item.sortable({
					items:".appButton",
					connectWith :folderitems.find(".folderInner"),					
					opacity :"0.6",
					stop: function(event, ui) {
						var item = ui.item;
						var p = item.parent();
...
Task = $.Class({
	...
	create:function(op)	{
		...
		$("<img src='icon/min/"+op.icon+"'/><div class='taskItemIconState'></div>").appendTo(taskItemIcon);
		var taskItemTxt  = $("<div>",{
			"class":"taskItemTxt",
			text : op.title
		});
